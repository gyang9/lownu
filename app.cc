...
int main(int argc, char**argv) {
    ...
    //TH1D test = rep->TrueNuEShift(1);
    //std::cout << "test.GetMean()   : " << test.GetMean()   << std::endl;
    //std::cout << "test.GetStdDev() : " << test.GetStdDev() << std::endl;

    std::vector<TH1D> tempPredList = rep->preparePrediction(rep->getPullList(), false);
    std::vector<TH1D> tempPredLis = rep->test(rep->getPullList(), false);
    std::cout << tempPredList.at(0).GetEntries() << std::endl;;
