...
std::vector<TH1D*> Sterile:: preparePrediction(RooListProxy* _pulls, bool Iosc) const
{
    ...
    float recoNuE;
    float trueNuE;
    TFile file(fileLocation + "variableOutput.root");
    TTree* tree = (TTree*)file.Get("tree");
    TFile file2(fileLocation + "flux_shifts.root");
    TDirectory *dir = file2.GetDirectory("syst0");
    TH1D* ND_numubar_RHC;
    dir->GetObject("ND_numubar_RHC",ND_numubar_RHC);

    tree->SetBranchAddress("recoNeutrinoE", &recoNuE);
    tree->SetBranchAddress("trueNeutrinoE", &trueNuE);
    TH1D* predNuE = new TH1D("","",16,0,8);
    for (int i = 0; i < tree->GetEntries(); i++) {
        int temp = 0;
        for (int i = 1; i < ND_numubar_RHC->GetNbinsX(); i++)
        {
            if (ND_numubar_RHC->GetBinLowEdge(i) + ND_numubar_RHC->GetBinWidth(i) > trueNuE && ND_numubar_RHC->GetBinLowEdge(i) < trueNuE) {
                temp = i;
            }
        }
        predNuE->Fill(recoNuE, 1 + ((RooAbsReal*)_pulls->at(i+12))->getVal() * ND_numubar_RHC->GetBinContent(temp));
    }
...
std::vector<TH1D*> Sterile:: prepareData(std::vector<TH1D*> tempPredList) const// 
{
    ...
    TTree* tree = (TTree*)file.Get("tree");
    tree->SetBranchAddress("recoNeutrinoE", &recoNuE);
    TH1D* dataNuE = new TH1D("","",16,0,8);
    for (int i = 0; i < tree->GetEntries(); i++) {
        dataNuE->Fill(recoNuE);
    }
    dataList.push_back(dataNuE);
